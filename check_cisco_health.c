		...
		switch(ptr->table->state)
		{
			case 1:
				...
				if ((ptr->type==0 || ptr->type==1) && (ptr->table->value > 0))
				{
					...
					/* now add perfdata */
					addstr(&perfstrp, &perfstrsize, " %s_%d=%d%c", table_names[ptr->type], ptr->index, ptr->table->value, ptr->type?'C':'V');
				} else {
...
int addval(int env, int index, int var, netsnmp_variable_list *result)
{
	...
	for (ptr=table_listp; ptr; ptr=ptr->next)
	{
		if (ptr->type == env && ptr->index == index) { break; }
		last = ptr;
	}
	if (!ptr)
	{
...
{
	static unsigned char allowed[256] = "_________________________________!_#_%__()*+,-.-0123456789___=_?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^__abcdefghijklmnopqrstuvwxyz{_}__________________________________________________________________________________________________________________________________";
